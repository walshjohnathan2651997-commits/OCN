...
**Version:** V3.5 (presentation ready)
**Date:** 2026-07-04
**Changes from V3.4:** Presentation pass — abstract tightened to ~200 words, introduction compressed to six paragraphs, Method/Problem Definition fitted with Figures 1-2, Results fitted with Figure 3 and three clean main tables, Discussion restructured into four subsections, detail moved to appendix case studies, submission snapshot added. No new experiments, no API calls, no new data, no change to conclusions.
...
![Figure 2: Taxonomy Decision Tree](figure_2_taxonomy_decision_tree.md)
...
The decision tree is a *labeling aid*, not a gold standard. Cases where the tree yields ambiguous results are flagged as boundary cases for future gold adjudication (§V.D, `high_risk_sample_bank.csv`). Table 2 (§VI) provides 8 illustrative examples (2 per class) from the SimClaim silver set; full versions are in `table_taxonomy_examples_clean.csv`.
...
![Figure 1: Method Flow](figure_1_method_flow.md)
...
![Figure 3: LLM vs R4 strong gap](figure_3_llm_vs_r4_strong_gap.md)
...
See `appendix_case_studies.md` for 8 detailed case studies: 4 trusted cases (one per class) and 4 boundary cases (mild vs. strong, strong vs. contradiction, supported vs. mild, evidence too short / context missing). Boundary cases are used to illustrate task difficulty, not to support strong claims.
