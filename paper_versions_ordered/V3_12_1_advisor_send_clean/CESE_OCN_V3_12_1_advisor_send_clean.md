...
**Version:** V3.12.1 Advisor Send Clean (validation-pending, protocol-locked)
**Date:** 2026-07-04
...
**Changes from V3.12:** Encoding clean and minor wording stabilization only — no experimental changes, no structural changes. Mojibake scan confirmed 0 hits (V3.12 was already clean UTF-8 with proper Unicode characters §, κ, Δ, ≥, −, —, –). Two occurrences of the adverb "systematically" (in the Abstract and §I, qualifying the LLM strong_action under-detection pattern) were softened to "consistently"; the qualifier "under the tested prompts" is already present in both contexts, and the strong_action under-detection main conclusion is preserved. **No change to any experimental number, no change to §VIII.D, no change to the non-gold caveat, no change to the 50% retention caveat, no change to the R4 macro-F1 < LLM fact, no change to the gold-not-done fact.** V3.12, V3.11, V3.10.1, and V3.9 are preserved unchanged.
...
The remainder of the paper is organized as follows: §II positions the work; §III formulates the task and provides taxonomy examples (Table 0) and the decision tree (Figure 2); §IV describes the CESE-OCN/R4 framework (Figure 1); §V describes the data and protocol; §VI reports controlled silver-stage results (Figure 3, Tables 1–3); **§VII specifies the pre-registered gold validation protocol, including placeholder Tables G1–G3 and frozen success/downgrade criteria**; §VIII discusses implications, including three non-gold credibility checks (§VIII.D) that test evidence necessity, screening utility, and label-shift robustness before gold adjudication; §IX states limitations; §X concludes. Case studies and detailed tables are in the appendix.
