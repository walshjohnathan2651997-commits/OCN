"""Build V3.5 docx from markdown using zipfile + XML (python-docx unavailable)."""
import os
import re
import zipfile
from xml.sax.saxutils import escape

MD_PATH = r"D:\ocn\paper_versions_ordered\V3_5_presentation_ready\CESE_OCN_V3_5_presentation_ready.md"
DOCX_PATH = r"D:\ocn\paper_versions_ordered\V3_5_presentation_ready\CESE_OCN_V3_5_presentation_ready.docx"
...
def table_block(rows):
    ...
    for i, row in enumerate(rows):
        ...
        for j in range(n_cols):
            ...
            cells_xml.append(f'<w:tc><w:tcPr><w:tcW w:w="2000" w:type="dxa"/></w:tcPr><w:p>{ppr}{run}</w:p></w:tc>')
...
def parse_markdown(md):
    ...
    while i < len(lines):
        ...
        if stripped.startswith('```'):
            if in_code_block:
                elements.append('<w:p><w:pPr><w:pStyle w:val="TableCell"/></w:pPr><w:r><w:rPr><w:rFonts w:ascii="Consolas" w:hAnsi="Consolas"/></w:rPr><w:t xml:space="preserve">--- end code block ---</w:t></w:r></w:p>')
                in_code_block = False
            else:
                in_code_block = True
                elements.append('<w:p><w:pPr><w:pStyle w:val="TableCell"/></w:pPr><w:r><w:rPr><w:rFonts w:ascii="Consolas" w:hAnsi="Consolas"/></w:rPr><w:t xml:space="preserve">--- code block ---</w:t></w:r></w:p>')
            i += 1
            continue
        if in_code_block:
            safe = escape(stripped)
            elements.append(f'<w:p><w:pPr><w:pStyle w:val="TableCell"/></w:pPr><w:r><w:rPr><w:rFonts w:ascii="Consolas" w:hAnsi="Consolas"/></w:rPr><w:t xml:space="preserve">{safe}</w:t></w:r></w:p>')
            i += 1
            continue
        if not stripped:
            i += 1
        ...
        if stripped.startswith('- '):
            elements.append(f'<w:p><w:pPr><w:pStyle w:val="ListBullet"/></w:pPr>{render_inline_runs(stripped[2:])}</w:p>')
            i += 1; continue
        if stripped.startswith('✅') or stripped.startswith('❌'):
            elements.append(para(stripped)); i += 1; continue
        m = re.match(r'^(\d+)\.\s+(.*)', stripped)
        if m:
            elements.append(f'<w:p><w:pPr><w:pStyle w:val="ListNumber"/></w:pPr>{render_inline_runs(m.group(2))}</w:p>')
            i += 1; continue
...
def build_docx():
    ...
    print(f"DOCX written: {DOCX_PATH} ({os.path.getsize(DOCX_PATH)} bytes)")
