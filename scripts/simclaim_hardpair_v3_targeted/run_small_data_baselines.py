...
SPLIT_ROOT = ROOT / "data" / "simclaim_hardpair_v3_targeted" / "splits_group_stratified"
EXP_ROOT = ROOT / "experiments" / "simclaim_hardpair_v3_targeted_small_baselines"
METRIC_DIR = EXP_ROOT / "metrics"
PRED_DIR = EXP_ROOT / "predictions"
REPORT_DIR = EXP_ROOT / "reports"

METRICS_CSV = METRIC_DIR / "v3_targeted_small_data_baseline_metrics.csv"
PER_CLASS_CSV = METRIC_DIR / "v3_targeted_small_data_baseline_per_class_f1.csv"
PRED_CSV = PRED_DIR / "v3_targeted_small_data_baseline_predictions.csv"
SUMMARY_JSON = REPORT_DIR / "v3_targeted_small_data_baseline_summary.json"
REPORT_MD = REPORT_DIR / "v3_targeted_small_data_baseline_report.md"
    ...
    for target in TARGETS:
        for split, eval_rows in eval_sets.items():
            for model in ["majority", "claim_length_median_rule", "prefix_first4_memorize"]:
                ...
                row, pcs, preds = lib.score_run("simclaim_hardpair_v3_targeted_268_group_stratified", target, split, model, "claim_only", train_rows, eval_rows, y_pred)
            ...
            for view in TEXT_VIEWS:
                for model in ["tfidf_centroid", "multinomial_nb"]:
                    y_pred = lib.tfidf_centroid_predict(train_rows, eval_rows, target, view) if model == "tfidf_centroid" else lib.multinomial_nb_predict(train_rows, eval_rows, target, view)
                    row, pcs, preds = lib.score_run("simclaim_hardpair_v3_targeted_268_group_stratified", target, split, model, view, train_rows, eval_rows, y_pred)
                    metric_rows.append(row); per_class_rows.extend(pcs); pred_rows.extend(preds)
...
def write_report(metric_rows):
    ...
    SUMMARY_JSON.write_text(json.dumps({
        "dataset": "simclaim_hardpair_v3_targeted_268_group_stratified",
    ...
    lines = ["# Small-data baseline report: v3 targeted repair", "", "| target | claim-only F1 | claim+evidence F1 | CE - claim |", "|---|---:|---:|---:|"]
    for target in TARGETS:
        ...
        lines.append(f"| {target} | {claim['macro_f1']} | {ce['macro_f1']} | {float(ce['macro_f1'])-float(claim['macro_f1']):.6f} |")
