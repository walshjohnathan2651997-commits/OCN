    ...
    chain = [
        {
            ...
            "risk": "Oracle recall is on silver labels only",
        },
        {
        ...
        },
        {
            ...
            "risk": "Silver labels only; not human-audited",
        },
        {
            ...
            "risk": "Threshold not fitted on test; silver labels only",
        },
        {
        ...
        {
            "claim": "PDF-start extraction has results",
            "evidence_file": "experiments/pdf_extraction_stress_test_v1/",
            "status": "missing",
            "safe_wording": "Not yet implemented",
            "risk": "No PDF extraction stress test has been run",
        },
        {
            "claim": "Human audit has results",
            "evidence_file": "data/simclaim_all92_candidate_pool_v1/strict_silver_max_v1/strict_silver_max_candidates_v1.csv",
            "status": "partial",
            "safe_wording": "human_audited=False for all 444 candidates; no gold labels exist",
            "risk": "No human audit has been performed; all labels are silver",
        },
    ]
    return chain
...
def build_priority_actions(scripts: dict, experiments: dict, leakage: dict, mainline: dict) -> list:
    ...
    if mainline["old_narratives"]:
        actions.append({
            ...
            "reason": f"{len(mainline['old_narratives'])} old narrative references found",
        })
    ...
    if not experiments.get("bm25_sentence_retrieval_v1", {}).get("exists", False):
        actions.append({
            "priority": "P0",
            "action": "Run BM25 sentence retrieval on real data (not just toy)",
            "reason": "Only toy data exists; real retrieval needed for paper",
        })

    if not experiments.get("canonicalizer_ablation_v1", {}).get("exists", False):
        actions.append({
            "priority": "P0",
    ...
    if not experiments.get("format_shift_ablation_v1", {}).get("exists", False):
        actions.append({
            "priority": "P0",
    ...
    if not experiments.get("leakage_audit_v1", {}).get("exists", False):
        actions.append({
            "priority": "P0",
    ...
    if not experiments.get("lightweight_smart_queue_v1", {}).get("exists", False):
        actions.append({
            "priority": "P1",
    ...
    if leakage["hardcoded_d_ocn_paths"]:
        actions.append({
            ...
            "reason": f"{len(leakage['hardcoded_d_ocn_paths'])} scripts with hardcoded paths",
        })

    if scripts["B_proposed"].get("run_complexity_vs_utility_ablation_v1.py", {}).get("status") == "missing":
        actions.append({
            "priority": "P1",
    ...
    if scripts["B_proposed"].get("run_pdf_extraction_stress_test_v1.py", {}).get("status") == "missing":
        actions.append({
            "priority": "P1",
    ...
    actions.append({
        "priority": "P2",
        "action": "Generate final paper PDF from paper_assets",
...
def write_status_md(mainline: dict, scripts: dict, experiments: dict,
    ...
    lines.append("")
    lines.append("| Claim | Evidence File | Status | Safe Wording | Risk |")
    lines.append("|---|---|---|---|---|")
    for item in evidence_chain:
        lines.append(
            f"| {item['claim']} | `{item['evidence_file']}` | "
            f"{item['status']} | {item['safe_wording']} | {item['risk']} |"
        )
