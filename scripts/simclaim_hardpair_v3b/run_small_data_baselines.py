...
SPLIT_ROOT = ROOT / "data" / "simclaim_hardpair_v3b" / "splits_group_stratified"
EXP_ROOT = ROOT / "experiments" / "simclaim_hardpair_v3b_small_baselines"
METRIC_DIR = EXP_ROOT / "metrics"
PRED_DIR = EXP_ROOT / "predictions"
REPORT_DIR = EXP_ROOT / "reports"

METRICS_CSV = METRIC_DIR / "hardpair_v3b_small_data_baseline_metrics.csv"
PER_CLASS_CSV = METRIC_DIR / "hardpair_v3b_small_data_baseline_per_class_f1.csv"
PRED_CSV = PRED_DIR / "hardpair_v3b_small_data_baseline_predictions.csv"
SUMMARY_JSON = REPORT_DIR / "hardpair_v3b_small_data_baseline_summary.json"
REPORT_MD = REPORT_DIR / "hardpair_v3b_small_data_baseline_report.md"
    ...
    for target in TARGETS:
        for split, eval_rows in eval_sets.items():
            for model in ["majority", "claim_length_median_rule", "prefix_first4_memorize"]:
                ...
                row, pcs, preds = lib.score_run("simclaim_hardpair_v3b_268_group_stratified", target, split, model, "claim_only", train_rows, eval_rows, y_pred)
            ...
            for view in TEXT_VIEWS:
                for model in ["tfidf_centroid", "multinomial_nb"]:
                    ...
                    row, pcs, preds = lib.score_run("simclaim_hardpair_v3b_268_group_stratified", target, split, model, view, train_rows, eval_rows, y_pred)
                    metric_rows.append(row); per_class_rows.extend(pcs); pred_rows.extend(preds)
...
def write_report(metric_rows):
    ...
    SUMMARY_JSON.write_text(json.dumps({
        "dataset": "simclaim_hardpair_v3b_268_group_stratified",
    ...
    lines = ["# Small-data baseline report: hardpair v3b 268", "", "## Best test result by target", "", "| target | best model | view | acc | macro-F1 | positive F1 |", "|---|---|---|---:|---:|---:|"]
