...
**Date:** 2026-07-04
**Changes from V3.6:** Final cleanup pass — fixed `EMNAACP` typos in sibling files (not in main paper); unified table numbering (Table 0 = taxonomy examples, Table 1 = internal R4, Table 2 = LLM comparison, Table 3 = boundary audit — no duplicate numbering); changed "Author information to be added" to "Author information omitted for review draft"; verified no mojibake/replacement characters in main paper; generated advisor cover note, one-page summary, reviewer risk brief, cleanup log, and file manifest. No new experiments, no API calls, no new data, no change to numeric results, no change to conclusions.
